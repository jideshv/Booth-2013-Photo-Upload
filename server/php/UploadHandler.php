...
class UploadHandler
{
    ...
    protected $error_messages = array(
        ...
        'max_file_size' => 'Pictures must be below 20MB',
        'min_file_size' => 'File is too small',
        'accept_file_types' => 'Filetype not allowed',
        'max_number_of_files' => 'Only 500 pictures may be uploaded per user',
        'max_width' => 'Image exceeds maximum width',
        'min_width' => 'Please provide a 2 megapixel or above picture',
        'max_height' => 'Image exceeds maximum height',
        'min_height' => 'Please provide a 2 megapixel or above picture'
    );

    function __construct($options = null, $initialize = true) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']).'/files/SIN1/',
            'upload_url' => $this->get_full_url().'/files/SIN1/',
            'user_dirs' => true,
            'mkdir_mode' => 0774,
            'param_name' => 'files',
            ...
            'accept_file_types' => '/\.(jpe?g|png|3gp|avi|mov|m4v|mp4|mpg|wmv|mpeg)$/i',
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        if (!empty($version)) {
            ...
            if (!is_dir($version_dir)) {
                mkdir($version_dir, 0775, true);
                chgrp($version_dir,'ec2-user');
            }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            file_put_contents($file_path.'.txt',$file->caption);
            chmod ($file_path,0664);
            chmod ($file_path.'.txt',0664);
            chgrp ($file_path,'ec2-user');
            chgrp ($file_path.'.txt','ec2-user');
            $file->size = $file_size;
            $this->set_file_delete_properties($file);
        }
        return $file;
